// app.jsx — main orchestrator. window.App
(function () {
  const { isFolder, hostOf } = window.BMData;

  function useClock() {
    const [now, setNow] = React.useState(new Date());
    React.useEffect(() => { const t = setInterval(() => setNow(new Date()), 1000 * 20); return () => clearInterval(t); }, []);
    return now;
  }
  function greeting(h) {
    if (h < 5) return '夜深了';
  ...
  function Search({ store, onOpenBookmark, onEnterFolder }) {
    ...
    const results = React.useMemo(() => store.search(q), [q]);
    ...
    const pick = (n) => { if (isFolder(n)) onEnterFolder(n); else onOpenBookmark(n); setQ(''); setShow(false); };
    const onKey = (e) => {
    ...
    return (
      <div className="searchwrap" ref={wrapRef}>
        <div className="search glass">
          ...
          <span className="kbd">{results.length ? results.length : ''}</span>
        </div>
        {open && (
              ...
              : results.map((n, i) => (
                <div key={n.id} className={'res-item' + (i === active ? ' active' : '')}
                  ...
                  <div className="res-meta">
                    ...
                    <div className="res-path">{isFolder(n) ? '文件夹' : hostOf(n.url)} · {store.pathString(n.id)}</div>
                  </div>
                </div>
              ))}
          </div>
  ...
  window.App = function App() {
    ...
    const [dialog, setDialog] = React.useState(null); // {type, node}
    const [settingsOpen, setSettingsOpen] = React.useState(false);
    const [pop, setPop] = React.useState(null); // {x,y,node}

    const theme = store.theme(), density = store.layout(), accent = store.accent();
    const wp = store.wallpaper(), f = store.filter(), col = store.color();
    const radius = store.read('bookmark_radius', 18), gblur = store.read('bookmark_glassblur', null);

    // apply globals
    React.useEffect(() => {
      const el = document.documentElement;
      el.dataset.theme = theme; el.dataset.density = density;
    ...
    React.useEffect(() => {
      ...
      else if (theme === 'light') layers.push(flat('#eef1f6'));
      layers.push(window.WALLPAPERS[wp] || window.WALLPAPERS[0]);
    ...
    const menuItems = (node) => ([
      ...
      { icon: isFolder(node) ? 'folder' : 'upload', label: isFolder(node) ? '打开文件夹' : '在新标签页打开', onClick: () => isFolder(node) ? goFolder(node) : openBookmark(node) },
      { sep: true },
    ...
    const submitAdd = ({ title, url, tags }) => { store.addBookmark(currentId, title, url, tags); setDialog(null); window.toast('书签已添加', 'check'); };
    const submitFolder = (name) => { store.addFolder(currentId, name); setDialog(null); window.toast('文件夹已创建', 'folderPlus'); };
    const submitEdit = (fields) => { store.update(dialog.node.id, fields); setDialog(null); window.toast('已保存', 'check'); };
    const doDelete = () => {
      const n = dialog.node;
      store.remove(n.id); setDialog(null);
      window.toast((isFolder(n) ? '文件夹' : '书签') + '已删除', 'trash');
    };
    const batchDelete = () => { store.removeMany(selected); window.toast('已删除 ' + selected.length + ' 项', 'trash'); setSelected([]); setSelectMode(false); };

    return (
      <div className="app">
        <div className="wrap">
          <div className="hero">
            <div className="clock">{now.getHours().toString().padStart(2, '0')}:{now.getMinutes().toString().padStart(2, '0')}</div>
              ...
              {activeTag ? (
                ...
                <React.Fragment key={n.id}>
                  ...
                  <span className={'crumb' + (i === crumbs.length - 1 ? ' current' : '')}
                    onClick={() => i < crumbs.length - 1 && setCurrentId(n.id)}>{i === 0 ? '书签栏' : n.title}</span>
                </React.Fragment>
              ))}
            </div>
            <div className="tool-actions">
              <button className={'iconbtn' + (selectMode ? ' active' : '')} onClick={() => { setSelectMode((s) => !s); setSelected([]); }} title="多选">
                <window.Icon name="select" size={16} />{selectMode ? '完成' : '选择'}
              </button>
              <button className="iconbtn icon-only" style={{ width: 38 }} onClick={() => setDialog({ type: 'io' })} title="导入 / 导出"><window.Icon name="download" size={17} /></button>
              <button className="iconbtn icon-only" style={{ width: 38 }} onClick={() => setSettingsOpen(true)} title="设置"><window.Icon name="gear" size={18} /></button>
            </div>
          </div>
          ...
          <Grid store={store} currentId={currentId} items={items}
            ...
            onAddBookmark={() => setDialog({ type: 'add' })} onNewFolder={() => setDialog({ type: 'folder' })}
        ...
        {selectMode && selected.length > 0 && (
          <div className="selbar glass">
            ...
            <button className="iconbtn" onClick={() => setDialog({ type: 'move' })}><window.Icon name="move" size={16} />移动</button>
            <button className="iconbtn" onClick={batchDelete}><window.Icon name="trash" size={16} />删除</button>
            <button className="iconbtn" onClick={() => { setSelectMode(false); setSelected([]); }}><window.Icon name="close" size={16} /></button>
          </div>
        )}

        {pop && <window.PopMenu x={pop.x} y={pop.y} items={menuItems(pop.node)} onClose={() => setPop(null)} />}

        {dialog && dialog.type === 'add' && <window.AddBookmarkDialog folderName={current.title === '书签栏' || isRoot ? '书签栏' : current.title} onClose={() => setDialog(null)} onSubmit={submitAdd} />}
        {dialog && dialog.type === 'folder' && <window.FolderDialog folderName={isRoot ? '书签栏' : current.title} onClose={() => setDialog(null)} onSubmit={submitFolder} />}
        {dialog && dialog.type === 'edit' && <window.EditDialog node={dialog.node} onClose={() => setDialog(null)} onSubmit={submitEdit} />}
        {dialog && dialog.type === 'confirm' && <window.ConfirmDialog
          ...
          confirmLabel="删除" danger onClose={() => setDialog(null)} onConfirm={doDelete} />}
        {dialog && dialog.type === 'io' && <window.ImportExportDialog store={store} currentId={currentId} onClose={() => setDialog(null)} onDone={() => {}} />}
        {dialog && dialog.type === 'move' && <window.MoveDialog store={store} ids={selected} onClose={() => setDialog(null)}
          onDone={(targetId, nm) => { store.moveMany(selected, targetId); window.toast('已移动 ' + selected.length + ' 项到「' + nm + '」', 'move'); setSelected([]); setSelectMode(false); setDialog(null); }} />}

        <window.SettingsPanel open={settingsOpen} onClose={() => setSettingsOpen(false)} store={store} />
        <window.ToastHost />
        <window.Tweaks store={store} />
      </div>
    );
  };
})();
