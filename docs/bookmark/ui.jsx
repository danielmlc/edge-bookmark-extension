// ui.jsx — icons, favicon tiles, wallpapers, toast system. Exposed on window.
(function () {
  const { brandFor, letterFor, isFolder, hostOf } = window.BMData;
  ...
  const P = {
    ...
    gear: 'M12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8zm9 4c0-.5 0-1-.1-1.4l2-1.6-2-3.4-2.4 1a7.6 7.6 0 0 0-2.4-1.4L13.7 2h-3.4l-.4 2.6c-.9.3-1.7.8-2.4 1.4l-2.4-1-2 3.4 2 1.6c-.1.4-.1.9-.1 1.4s0 1 .1 1.4l-2 1.6 2 3.4 2.4-1c.7.6 1.5 1.1 2.4 1.4l.4 2.6h3.4l.4-2.6c.9-.3 1.7-.8 2.4-1.4l2.4 1 2-3.4-2-1.6c.1-.4.1-.9.1-1.4z',
    ...
    sun: 'M12 7a5 5 0 1 0 0 10 5 5 0 0 0 0-10zm0-5v3m0 14v3m9-9h-3M5 12H2m15.5-6.5-2 2M8.5 15.5l-2 2m11 0-2-2M8.5 8.5l-2-2',
  };
  window.Icon = function Icon({ name, size = 20, stroke }) {
    const d = P[name] || '';
    if (name === 'sun') {
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
          <circle cx="12" cy="12" r="4" /><path d="M12 2v3m0 14v3m9-9h-3M5 12H2m15.5-6.5-2 2M8.5 15.5l-2 2m11 0-2-2M8.5 8.5l-2-2" />
        </svg>
      );
    }
  ...
  window.Favicon = function Favicon({ node, size = 44, radius }) {
    if (isFolder(node)) {
      return (
        <div className="fav folder" style={{ width: size, height: size, borderRadius: radius || size * .32 }}>
    ...
    return (
      <div className="fav" style={{ width: size, height: size, borderRadius: radius || size * .32, background: bg, color: fg, fontSize: Math.round(size * .42) }}>
        {letterFor(node)}
      </div>
    );
  };

  // ---------- wallpapers (offline-safe gradient meshes) ----------
  window.WALLPAPERS = [
    // dawn
  ...
  window.PopMenu = function PopMenu({ x, y, items, onClose }) {
    ...
    React.useEffect(() => {
      const h = () => onClose();
      window.addEventListener('click', h); window.addEventListener('scroll', h, true); window.addEventListener('resize', h);
