// cards.jsx — bookmark grid, tiles, drag & drop, multi-select. window.Grid
(function () {
  ...
  function Tile({ node, onOpen, onMore, selectMode, selected, onToggle, drag, favSize }) {
    ...
    return (
      <div className={cls} draggable={!selectMode}
        onClick={(e) => { if (selectMode) { onToggle(node.id); } else onOpen(node); }}
  ...
  const FAV = { compact: 36, normal: 44, comfort: 56 };
  ...
  window.Grid = function Grid({ store, currentId, items, isRoot, selectMode, selectedIds, onToggleSelect,
    ...
    const handlers = {
      ...
      start: (e, node) => { dragRef.current.id = node.id; setDrag({ dragId: node.id, overId: null }); e.dataTransfer.effectAllowed = 'move'; try { e.dataTransfer.setData('text/plain', node.id); } catch (x) {} },
      end: () => { dragRef.current.id = null; setDrag({ dragId: null, overId: null }); },
      over: (e, node) => { if (!dragRef.current.id || dragRef.current.id === node.id) return; e.preventDefault(); if (drag.overId !== node.id) setDrag((d) => ({ ...d, overId: node.id })); },
      leave: () => {},
      drop: (e, node) => {
        e.preventDefault(); e.stopPropagation();
        const id = dragRef.current.id; dragRef.current.id = null;
        if (!id || id === node.id) { setDrag({ dragId: null, overId: null }); return; }
        if (isFolder(node)) { store.move(id, node.id, null); window.toast('已移入「' + node.title + '」', 'move'); }
        else { store.reorder(id, node.id); }
        setDrag({ dragId: null, overId: null });
      },
    };

    // back-card drop handlers
    const backDrop = {
      over: (e) => { if (!dragRef.current.id) return; e.preventDefault(); if (drag.overId !== '__back') setDrag((d) => ({ ...d, overId: '__back' })); },
      drop: (e) => {
        e.preventDefault(); const id = dragRef.current.id; dragRef.current.id = null;
        if (id) { const parent = store.node(currentId); const grand = parent && parent.parentId; if (grand) { store.move(id, grand, null); window.toast('已移到上级', 'arrowUp'); } }
        setDrag({ dragId: null, overId: null });
      },
    };
      ...
      <div className="grid">
        {!isRoot && (
          <div className={'tile back action' + (drag.overId === '__back' ? ' drop-target' : '')}
            onClick={onBack} onDragOver={backDrop.over} onDragLeave={handlers.leave} onDrop={backDrop.drop}>
            <window.Icon name="arrowUp" size={26} className="a-ic" />
        ...
        {items.length === 0 && (
          <div className="empty">
            ...
            <div className="e-sub">添加一个书签，或把别的卡片拖进来开始整理。</div>
        ...
        {items.length > 0 && !selectMode && !hideAdd && (
          <>
            <div className="tile action" onClick={onAddBookmark}>
              <window.Icon name="plus" size={26} className="a-ic" />
              <div className="t-title">添加书签</div>
            </div>
            <div className="tile action" onClick={onNewFolder}>
              <window.Icon name="folderPlus" size={26} className="a-ic" />
