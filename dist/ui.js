(function() {
  ...
  window.Icon = function Icon({ name, size = 20 }) {
    const d = P[name] || "";
    return /* @__PURE__ */ React.createElement("svg", { width: size, height: size, viewBox: "0 0 24 24", fill: "currentColor" }, /* @__PURE__ */ React.createElement("path", { d }));
  };
  window.Favicon = function Favicon({ node, size = 44, radius }) {
    const r = radius != null ? radius : Math.round(size * 0.32);
    if (isFolder(node)) {
      return /* @__PURE__ */ React.createElement("div", { className: "fav folder", style: { width: size, height: size, borderRadius: r } }, /* @__PURE__ */ React.createElement(window.Icon, { name: "folder", size: Math.round(size * 0.5) }));
    }
    const [bg, fg] = brandFor(node.url || "");
    const letter = letterFor(node);
    return /* @__PURE__ */ React.createElement("div", { className: "fav", style: { width: size, height: size, borderRadius: r, background: bg, color: fg, fontSize: Math.round(size * 0.42) } }, letter);
  };
  window.WALLPAPERS = [
  ...
  let _tid = 0;
  const tListeners = /* @__PURE__ */ new Set();
  let _toasts = [];
  function pushToast(msg, icon) {
    const id = ++_tid;
    _toasts = [..._toasts, { id, msg, icon }];
    tListeners.forEach((f) => f());
    setTimeout(() => {
      _toasts = _toasts.map((t) => t.id === id ? { ...t, out: true } : t);
      tListeners.forEach((f) => f());
      setTimeout(() => {
  ...
  window.ToastHost = function ToastHost() {
    ...
    React.useEffect(() => {
      tListeners.add(force);
      return () => tListeners.delete(force);
    }, []);
    return /* @__PURE__ */ React.createElement("div", { className: "toast-host" }, _toasts.map((t) => /* @__PURE__ */ React.createElement("div", { key: t.id, className: "toast" + (t.out ? " out" : "") }, t.icon && /* @__PURE__ */ React.createElement(window.Icon, { name: t.icon, size: 16 }), /* @__PURE__ */ React.createElement("span", null, t.msg))));
  };
  window.PopMenu = function PopMenu({ x, y, items, onClose }) {
    ...
    return /* @__PURE__ */ React.createElement("div", { ref, className: "popmenu glass", style: { left: pos.left, top: pos.top }, onClick: (e) => e.stopPropagation() }, items.map(
      (it, i) => it.sep ? /* @__PURE__ */ React.createElement("div", { key: i, className: "sep" }) : /* @__PURE__ */ React.createElement("button", { key: i, className: it.danger ? "danger" : "", onClick: () => {
        onClose();
        it.onClick();
      } }, /* @__PURE__ */ React.createElement(window.Icon, { name: it.icon, size: 16 }), " ", it.label)
    ));
  };
})();
