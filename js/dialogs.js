...
(function () {
  ...
  function Overlay({ children, onClose, shake }) {
    return (
      <div className="overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
        <div className={'dialog' + (shake ? ' shake' : '')} onMouseDown={(e) => e.stopPropagation()}>
          {children}
        </div>
      </div>
    );
  ...
  window.EditDialog = function ({ node, onClose, onSubmit }) {
    ...
    return (
      <Overlay onClose={onClose} shake={shake}>
        ...
        <div className="field"><label>名称</label>
          <input ref={ref} className="input" value={title}
            onChange={(e) => setTitle(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit()} /></div>
        {!folder && (
          <div className="field"><label>网址</label>
  ...
  window.ImportExportDialog = function ({ onClose, store, currentId }) {
    ...
    const doImport = async () => {
      ...
      if (ok) { window.toast('导入成功', 'check'); onClose(); }
      ...
      <Overlay onClose={onClose}>
        ...
        {tab === 'export' ? (
          ...
          <>
            ...
            <textarea className="input" value={text} placeholder='[{"title":"示例","url":"https://example.com"}]'
              onChange={(e) => setText(e.target.value)} style={{ minHeight: 200 }} />
            <div className="dlg-actions">
              <button className="btn" onClick={onClose}>取消</button>
  ...
  window.MoveDialog = function ({ store, ids, onClose, onDone }) {
    ...
    ids.forEach((id) => {
      (function mark(n) { if (!n) return; blocked.add(n.id); (n.children || []).forEach(mark); })(store.node(id));
    });
    const rows = [];
        ...
        <div style={{ maxHeight: 280, overflowY: 'auto', margin: '0 -6px' }}>
          {rows.map(({ node, depth }) => (
            <button key={node.id} style={{
              ...
              justifyContent: 'flex-start', border: 0, background: 'transparent',
              height: 42, paddingLeft: 10 + depth * 18, fontWeight: 500,
              cursor: 'pointer', borderRadius: 8, color: 'var(--text)', fontFamily: 'inherit', fontSize: 13.5
            }}
              onMouseEnter={(e) => e.currentTarget.style.background = 'var(--card-hover)'}
              onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
              onClick={() => onDone(node.id, node.id === '1' ? '书签栏' : node.title)}>
              <window.Icon name="folder" size={18} />
